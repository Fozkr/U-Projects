...
#include <sys/file.h>
// Includes  agregados por farroyo. abajo de esta linea
#include <sys/fcntl.h>
#include <unistd.h>
#include <stdlib.h>
...
void abort();
//void exit();
// La linea original es la siguiente, cambiada por farroyo
// int mprotect( char *addr, int len, int prot);
// linea agregada, por farroyo, abajo
//int mprotect(char *addr, unsigned int len, int prot);

//int bind ( int, const struct sockaddr *, unsigned int);
//int recvfrom (int, void*, unsigned int, int, struct sockaddr *, unsigned int *);
//int sendto (int, const void*, unsigned int, int, const struct sockaddr *, unsigned int);
}
...
// poll file or socket
//#IFDEF HOST_i386
// HOST_SPARC agregada por farroyo
#if defined(HOST_i386) || defined(HOST_SPARC)
...
//	management calls.
//	It uses the SUN specific sysconf system call
//      The page size is a system page size and need not to be the same
//	as the underlying hardware page size
// Function added by farroyo
//----------------------------------------------------------------------
